    ...
    <div class="layui-layout layui-layout-admin">
        ...
        <div class="layui-side layui-side-menu">
            <div class="layui-side-scroll">
                <div class="layui-logo" lay-href="${path}/home">
                    <span>后台管理系统</span>
                </div>
                    ...
                    <c:forEach items="${menus}" var="menu">
                        <c:if test="${ menu.pid eq '1'}">
                            <li data-name="${menu.id}" class="layui-nav-item">
                                <a href="javascript:;"
                                        <c:if test="${!empty menu.href}">lay-href="${path}${menu.href}"</c:if>
                                   lay-tips="${menu.name}" lay-direction="2">
                                    <i class="layui-icon ${menu.icon}"></i>
                                    <cite>${menu.name}</cite>
                                </a>
                            <c:forEach items="${menus}" var="menu1">
                                <c:if test="${menu1.pid eq menu.id}">
                                    <dl class="layui-nav-child">
                                        <dd data-name="${menu1.id}"   >
                                            <a href="javascript:;"
                                               <c:if test="${!empty menu1.href}"> lay-href="${path}${menu1.href}"</c:if>
                                                   >${menu1.name}</a>
                                            <c:if test="${empty menu1.href}">
                                            <dl class="layui-nav-child">
                                                <c:forEach items="${menus}" var="menu2">
                                                    <c:if test="${menu2.pid eq menu1.id}">
                                                        <dd data-name="${menu2.id}"><a lay-href="${path}${menu2.href}">${menu2.name}</a></dd>
                                                    </c:if>
                                                </c:forEach>
                                            </dl>
                                            </c:if>
